...
func (rsc *ReplicaSetController) syncReplicaSet(key string) error {
	...
	updatedRS, err := updateReplicaSetStatus(rsc.GetClient().AppsV1().ReplicaSets(rs.Namespace), rs, newStatus)
	if err != nil {
...
func (rsc *ReplicaSetController) claimPods(rs *apps.ReplicaSet, selector labels.Selector, filteredPods []*v1.Pod) ([]*v1.Pod, error) {
	...
	canAdoptFunc := controller.RecheckDeletionTimestamp(func() (metav1.Object, error) {
		fresh, err := rsc.GetClient().AppsV1().ReplicaSets(rs.Namespace).Get(rs.Name, metav1.GetOptions{})
		if err != nil {
			return nil, err
		}
		if fresh.UID != rs.UID {

...
func TestSyncReplicaSetDoesNothing(t *testing.T) {
	...
	manager.syncReplicaSet(testutil.GetKey(rsSpec, t))
...
func TestSyncReplicaSetCreateFailures(t *testing.T) {
	...
	manager.podControl = &fakePodControl
	manager.syncReplicaSet(testutil.GetKey(rs, t))
		...
		*(rsSpec.Spec.Replicas) = replicas
		...
		for i := 0; i < numReplicas; i += burstReplicas {
			...
			if replicas != 0 {
				...
				expectedDels := manager.expectations.GetUIDs(testutil.GetKey(rsSpec, t))
				podsToDelete := []*v1.Pod{}
				isController := true
			...
			if replicas != 0 {
				...
				expectedDel := manager.expectations.GetUIDs(testutil.GetKey(rsSpec, t))
...
func TestDoNotPatchPodWithOtherControlRef(t *testing.T) {
	...
	err := manager.syncReplicaSet(testutil.GetKey(rs, t))
	if err != nil {
		t.Fatal(err)
	}
